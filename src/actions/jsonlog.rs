...
fn value_to_string(v: &crate::abi::DecodedValue) -> String {
    ...
    match v {
        ...
        String(s) => s.clone(),
        Array(arr) => format!("[{}]", arr.iter().map(value_to_string).collect::<Vec<_>>().join(",")),
...
impl Action for JsonLogAction {
    ...
    fn on_tx(&self, t: &TxRecord) -> anyhow::Result<()> {
        let j = JsonTx {
            ...
            receipt_logs: t.receipt_logs.as_ref().map(|logs| logs.iter().map(|l| JsonReceiptLog {
    ...
    fn on_block(&self, b: &BlockRecord) -> anyhow::Result<()> {
        let j = JsonBlock { kind: "block", number: b.number };
        println!("{}", serde_json::to_string(&j)?);
        Ok(())
    }
}
