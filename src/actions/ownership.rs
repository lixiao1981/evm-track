        ...
        if let Some(name) = &e.name {
            if name == "OwnershipTransferred" || name == "OwnershipTransfer" {
                ...
                for f in &e.fields {
                    ...
                    match key.as_str() {
                        "previousowner" | "previous_owner" | "from" => previous = Some(format!("{:?}", f.value)),
                        "newowner" | "new_owner" | "to" => new_owner = Some(format!("{:?}", f.value)),
                        _ => {}
                    }
                }
                println!("[ownership] contract={} previous={:?} new={:?}", e.address, previous, new_owner);
            }
        }
        Ok(())
    }
