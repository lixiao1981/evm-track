...
pub async fn reset_stuck_jobs(pool: &PgPool) -> Result<u64, DbError> {
...
pub async fn count_pending_jobs(pool: &PgPool) -> Result<i64, DbError> {
...
pub async fn set_job_status(pool: &PgPool, hash: &str, status: i16) -> Result<(), DbError> {
...
pub async fn create_receipts_table(pool: &PgPool) -> Result<(), DbError> {
    sqlx::query(
        r#"
...
pub async fn insert_receipt(pool: &PgPool, receipt: &TransactionReceipt) -> Result<(), DbError> {
...
mod tests {
    ...
    async fn test_db_connection() -> Result<(), DbError> {
        dotenv::dotenv().ok();
